...
l2fwd_mac_updating(struct rte_mbuf *m, unsigned dest_portid)
{
	...
	//*((uint64_t *)tmp) = 0x6a1fe7a1420c ;//+ ((uint64_t)dest_portid << 40);
	*((uint64_t *)tmp) = 0x73fc0e62b8c2 ;//+ ((uint64_t)dest_portid << 40);
...
l2fwd_simple_forward(struct rte_mbuf *m, unsigned portid)
{
	...
	//EDIT: TOUCH AND DROP MBUF--NP
	unsigned i;
	data = rte_pktmbuf_mtod(m, long long int *);
	for(i=0; i<rte_pktmbuf_pkt_len(m)/sizeof(long long int); i++)
		dummy = data[i];
	rte_pktmbuf_free_seg(m);
	return;
	//rte_pktmbuf_read(m, sizeof(struct rte_ether_hdr)+sizeof(struct rte_ipv4_hdr),
